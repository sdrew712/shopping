...
let date1 = new Date("8/30/21");
let date2 = new Date("9/2/21");
const dateDiff = (date1, date2) => {
  let dayLength = 1000 * 60 * 60 * 24;
  console.log(Date.parse(date1));
  console.log(Date.parse(date2));
  let differenceMs = Date.parse(date1) - Date.parse(date2);
  ...
  try {
    ...
    if (prodKeys.length > 0) {
      for (let item of iterator_obj) {
        ...
        let prodObj = new Product(iterator_obj[item]);
  ...
  try {
    ...
    if (magProdKeys.length > 0) {
      for (let item of iterator_obj) {
        ...
        let magProdObj = new MagicProduct(iterator_obj[item]);
